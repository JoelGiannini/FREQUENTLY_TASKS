...
CT_BUILD_VERSION="19.1"
export CT_BUILD_VERSION
SCRIPT_VERSION="19.1"
SCRIPT_NAME="${0}"
...
checkSyntax() {
	...
	-o [full_path_dir_name]
		...
		be a full pathname. The file will be named Collection-<MACHINE_NAME>.tar.bz2.
	...
	-fastsearch	
		...
		searched. NOTE: The fastsearch option of Collection Tool
	...
	-p product	
		...
		Use \"all\" for the products that the Collection 
	...
	-r [file.txt|remote_user@remote_machine]
		Option to run the Collection Scripts remotely and gather
		results.  If used, the Collection tool will secure copy
	...
	-t [full_path_dir_name]
		Option to set the Collection Tool temporary output to files in the
		directory specified by full_path_dir_name. The option should
	...
	do
		...
		-d) SEARCH_DIR="$2" ;
			if [ ! -z "$SEARCH_DIR" ] ; then
				...
				if [ ! -d ${DIR} ] ; then
					$ECHO "\n$ARGUSAGE" >&2
					$ECHO "CT: CT-00001: ERROR: Argrument search directory $DIR does not exist or is not readable."
					exit 0
				fi
				done
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00002: ERROR: No argument provided to the -d parameter."
				exit 0
			fi
			;;
		-fsall) FS_SEARCH_ALL=$2 ;
			if [ ! -z "$FS_SEARCH_ALL" ] ; then
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00003: ERROR:  No argument provided to the -fsall parameter."				
				exit 0
			fi			
			;;
		-o) OUTPUT_DIR=$2 ;	
			if [ ! -z "$OUTPUT_DIR" ] ; then
				...
				if [ ! -w "${OUTPUT_DIR}" ] ; then
					$ECHO "\n$ARGUSAGE" >&2
					$ECHO "CT: CT-00004: ERROR:  User does not have the permission to write to the created subfolder, ${OUTPUT_DIR}."						
					exit 0
				fi			
				SCRIPT_OPTIONS="$SCRIPT_OPTIONS $1 $2"
				shift 2
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00005: ERROR:  No argument provided to the -o parameter."					
				exit 0
			fi				
			;;
		-follow) FOLLOW_ALL=$2 ;
			if [ ! -z "$FOLLOW_ALL" ] ; then		
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00006: ERROR:  No argument provided to the -follow parameter."	
				exit 0
			fi				
			;;	
		-debug) OUTPUT_DEBUG=$2 ;
			if [ ! -z "$OUTPUT_DEBUG" ] ; then		
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00007: ERROR:  No argument provided to the -debug parameter."	
				exit 0
			fi			
			;;	
		-L) LICAGREE=$2
			if [ ! -z "$LICAGREE" ] ; then	
				...
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00008: ERROR:  No argument provided to the -L parameter."					
				exit 0
			fi	
			;;
		-p) PRODLIST=$2
			...
			if [ -z "$PRODLIST" ] ; then	
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00009: ERROR:  No argument provided to the -p parameter."					
				exit 0
			fi
			...
			if [ "$PRODLIST" = "all" ] ;	then
				...
				do
					...
					if [ $? -ne 0 ] ; then
						$ECHO "\n$ARGUSAGE" >&2
						$ECHO "CT: CT-00010: ERROR:  $CHOSENPROD is not a valid product family."
						exit 0
					fi
		...
		-m) MASK_DATA=$2 ;
			if [ "$MASK_DATA" != "all" -a "$MASK_DATA" != "IP" -a "$MASK_DATA" != "ip" -a "$MASK_DATA" != "user" ] ; then
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00011: ERROR:  Valid option not chosen for the -m mask parameter."
				exit 0
			fi
			...
			if [ $? -ne 0 ] ; then
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00012: ERROR:  Perl was not found in the system path.\nPlease review the documentation for masking requirements."
				exit 0
			fi
			...
			if [ $? -ne 0 ] ; then
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00013: ERROR:  Cannot find a perl with Digest::SHA installed.\nPlease review the documentation for masking requirements."
				exit 0
			fi
		...
		-r) REMOTEINFO=$2
			...
			if [ -f "$REMOTEINFO" ] ; then	
				if [ "$REMOTEOPTIONUSED" = "true" ] ; then
					$ECHO "CT: CT-00022: ERROR:  Remote option argument cannot be specifed with the remote file and the user\@machine_info on the command line.  Please use one format for remote execution."
					exit 0
				else 
					RUNREMOTE_FILE="true"
					SCRIPT_OPTIONS="$SCRIPT_OPTIONS $1 $2"
				...
				case "$REMOTEINFO" in
				  ...
				  *)         
					$ECHO "\n$ARGUSAGE" >&2
					$ECHO "CT: CT-00014: ERROR:  Remote option argument must be in the form of user\@machine_info or a file containing informataion about multiple remote machines."
					exit 0
				esac
				SCRIPT_OPTIONS="$SCRIPT_OPTIONS $1 $2"
				shift 2
			...
			;;  			
		-t) CT_TMP=$2/ct_tmp_${MACHINE_NAME}_${CT_PID} ;
			if [ ! -z "$CT_TMP" ] ; then
				if [ ! -d "${CT_TMP}" ] ; then
					mkdir -p "${CT_TMP}" 
				fi			
				if [ ! -w "${CT_TMP}" ] ; then
					$ECHO "\n$ARGUSAGE" >&2
					$ECHO "CT: CT-00023: ERROR:  User does not have the permission to write to the created subfolder, ${CT_TMP}."						
					exit 0
				fi			
				mkdir -p $CT_TMP/logs
				CT_DEBUG=$CT_TMP/debug
				chmod o+x $CT_TMP
				CT_COLLECTED=$CT_TMP/logs/CT_collected.log
				CT_WARNINGS=$CT_TMP/logs/CT_warnings.log
				CT_ERRORS=$CT_TMP/logs/CT_errors.log
				UNIXCMDERR=$CT_TMP/logs/unixcmderrs.log
				touch $UNIXCMDERR
				SCRIPT_OPTIONS="$SCRIPT_OPTIONS $1 $2"
				shift 2
			else
				$ECHO "\n$ARGUSAGE" >&2
				$ECHO "CT: CT-00024: ERROR:  No argument provided to the -t parameter."					
				exit 0
			fi				
			;;	
			
		*) $ECHO "\n$ARGUSAGE" >&2
			$ECHO "CT: CT-00015: ERROR:  "$1" is an invalid entry."
			exit 0
	        ;;
		esac
	...
	if [ "$REMOTE_DB" = "YES" -a "$LICAGREE" = "YES" ] ; then
		$ECHO "\n$ARGUSAGE" >&2
		$ECHO "CT: CT-00016: ERROR:  -tns option cannot be used together with silent mode (-L Y)"
		exit 0
	fi 
	
	if [ "$RUNREMOTE_FILE" = "true" -a -n "$PRODLIST" ] ; then
		$ECHO "\n$ARGUSAGE" >&2
		$ECHO "CT: CT-00017: ERROR:  The -r option with the file argument cannot be used together with the -p option.\nPlease specify the -p option in the remote input file or see instructions for more detail."	
		exit 0
	fi 
	...
	if [ ! -w "${OUTPUT_DIR}" ] ; then
		$ECHO "\n$ARGUSAGE" >&2
		$ECHO "CT: CT-00004: ERROR:  User does not have the permission to write to the default output directory, ${OUTPUT_DIR}. Either chmod the correct permissions or chose an output directory via the -o command line arguement."			
		exit 0
	fi	
...
checkNFS () {
	...
	if [ $? -ne 0 ] ; then
		...
		if [ -n "$NFSDIRS" -a $SEARCH_NFS != "true" ] ; then
			echo_log "CT: CT-00110: WARNING: Current system has NFS drives mounted!\nIf you're sure that the Oracle products are installed only on local drives, continue with yes(y), otherwise select No(n) and please run it again with "-fsall true" option.\nRunning the Collection Tool on insufficient disks may have a significant impact on the quality of the data and information collected from this environment. Due to this, Oracle may have to get back to you and ask for additional items, or to execute again. " $CT_WARNINGS	
			...
			if [ "$LICAGREE" != "YES" ] ; then
				...
				if [ "${ANSWER}" = "n" ] ; then
					echo_log "CT-00018: ERROR: User chose not to continue the Collection Tool." $CT_ERRORS		
					exit 0
				fi
			fi
		fi
...
credentialValidation () {
	...
	if [ "${SCRIPT_USER}" != "root" ] ; then
		echo_log "CT: CT-00100: WARNING: Current OS user ${SCRIPT_USER} does NOT have 'administrative' rights!\nIf you're sure that the Current OS user ${SCRIPT_USER} is granted the required privileges, continue with yes(y), otherwise select No(n) and please log on with a OS user with sufficient privileges.\nRunning the Collection Tool with insufficient privileges may have a significant impact on the quality of the data and information collected from this environment. Due to this, Oracle may have to get back to you and ask for additional items, or to execute again." $CT_WARNINGS	
...
debugWarning () {
	...
	$ECHO_DEBUG "\ndebug.function.debugWarning"
		...
		echo_log "CT: CT-00200: WARNING: You have chosen to run the Collection tool in debug mode.\n  The script will write data in files in order for Oracle to debug the running of the scripts. You are required to inspect the files for any data that may be sensitive before returning the output to Oracle.  \nIf you wish to continue, continue with yes(y), otherwise select no(n) and contact your Oracle representative for more details." $CT_WARNINGS	
...
setOutputFiles() {
	...
	CT_FILES=$CT_TMP/logs/CTfiles.${CT_PID}
	CT_SORTED_FILES=$CT_TMP/logs/CTsortedfiles.${CT_PID}
	CT_MACHINFO_FILE=$CT_TMP/logs/${MACHINE_NAME}-info.txt
	CT_HOMES_FILE=$CT_TMP/logs/${MACHINE_NAME}-CT_Homes.txt
	CT_LOGS_FILE=$CT_TMP/logs/CTlogfiles.txt
	#files to be collected
	CT_TAR_FILE_LIST=$CT_TMP/logs/CTtarfilelist.txt
	CT_RMDATA_FILE_LIST=$CT_TMP/logs/CTRMDATAfilelist.txt
	
	CMDFILE=$CT_TMP/logs/OraCmdList
	CMDOUTFILE=$CT_TMP/logs/OraCmdOutFileList

	# debug and error files
	DEBUG_FILE=$CT_TMP/logs/CTdebugfile.${CT_PID}

	
	# bundled results file
	CT_TAR_FILE="${OUTPUT_DIR}/Collection-${MACHINE_NAME}${PRODUCTSRUN}.tar"
	export CT_TAR_FILE

	# results file
	CT_RESULTS_FILE=$CT_TMP/logs/Collection-${MACHINE_NAME}.txt	
	CT_ACTIONS_RESULTS_FILE=$CT_TMP/logs/Collection-Actions-${MACHINE_NAME}.txt	
...
getProducts() {
	...
	if [ -z "$PRODLIST" ] ; then
		...
		#MENUINCLUDELIST="CPUQ,OAS,SOA,Tuxedo,WLS"
		for PRODUCT in `ls ../resources/products/`
		do
			case $PRODUCT in
				CPUQ|OAS|SOA|WLS|DB|EBS|FMW|FormsReports|OBI|Webcenter )
					$ECHO "${PRODUCT}"
					;;
			esac
		...
		do
			...
			if [ "${PROD}" = "all" ] ; then 
				...
				do
					$ECHO "$PRODFAMILYLIST" | grep "$CHOSENPROD/" > /dev/null 2>&1
					if [ $? -ne 0 ] ; then
						echo_log "CT: CT-00010: ERROR:  $CHOSENPROD is not a valid product family." $CT_ERRORS
						continue
					fi
					PRODUCTSRUN="${PRODUCTSRUN}_${CHOSENPROD}"
					PRODLIST="$PRODLIST $CHOSENPROD"
...
getDefaultOracleEnv() {
	...
	for JAVAPROCESSLINE_TOKENS in `grep java ${CT_MACHINFO_FILE} | $CTAWK -F":| " -v OFS="\n" '$1=$1' `
	do
		case "$JAVAPROCESSLINE_TOKENS" in 
		*weblogic.jar*)
			echo $JAVAPROCESSLINE_TOKENS | awk -F'/' '{for (i=1; i<NF-3; i++) printf("%s/", $i)}' >> $CT_HOMES_FILE
			;;
		*.home=*)
			echo $JAVAPROCESSLINE_TOKENS | sed -e 's/.*.home=\(\S*\).*/\1/g' >> $CT_HOMES_FILE
			;;
		*_HOME=*)
			echo $JAVAPROCESSLINE_TOKENS | sed -e 's/.*._HOME=\(\S*\).*/\1/g' >> $CT_HOMES_FILE
			;;
		esac
	done
	...
	#Get java pids
	JAVAPIDS=`grep java ${CT_MACHINFO_FILE} | awk '{print $5}'`
	...
	do
		case $PID in
		   *[!0-9]*|'')
			...
			*)
				if [ "$OS_NAME" = "SunOS" ] ; then
						pargs -e $PID | grep $ENVGREPLIST  >> $CT_HOMES_FILE
				elif [ "$OS_NAME" = "AIX" ] ; then
						ps eauwww $PID | grep $ENVGREPLIST >> $CT_HOMES_FILE
				elif [ "$OS_NAME" = "Linux" ] ; then
						strings /proc/${PID}/environ | grep $ENVGREPLIST  >> $CT_HOMES_FILE
				fi
				;;
		esac
	...
	# Pair down CT_HOMES_FILE
	if [ -f $CT_HOMES_FILE ] ; then
		sed 's/user_projects*/ /g' $CT_HOMES_FILE > $CT_HOMES_FILE.tmp && mv $CT_HOMES_FILE.tmp $CT_HOMES_FILE
		sed 's/wlserver*/ /g' $CT_HOMES_FILE > $CT_HOMES_FILE.tmp && mv $CT_HOMES_FILE.tmp $CT_HOMES_FILE

		cat $CT_HOMES_FILE | sed -e 's/.*.weblogic\.jar\(\S*\).*/\1/g' -e 's/.*.home=\(\S*\).*/\1/g' -e 's/.*._HOME=\(\S*\).*/\1/g' | uniq > $CT_HOMES_FILE.tmp
		mv $CT_HOMES_FILE.tmp $CT_HOMES_FILE
	fi
}
################################################################################
#
...
doSearch() {
	...
	if [ -n "$SEARCH_EXCLUSIONS" ] ; then
		SEARCH_COMMAND="find ${SEARCH_DIR} \( ${SEARCH_EXCLUSIONS} \) ${PRUNE} -o \( ${ALLSEARCHFILES} \) ${SEARCH_OPTIONS} >> ${CT_FILES} 2>>$UNIXCMDERR"
		$ECHO_DEBUG "debug.search.command=$SEARCH_COMMAND"
		find ${SEARCH_DIR} \( ${SEARCH_EXCLUSIONS} \) ${PRUNE} -o \( ${ALLSEARCHFILES} \) ${SEARCH_OPTIONS} >> ${CT_FILES} 2>>$UNIXCMDERR &
		spinner
	else
		SEARCH_COMMAND="find ${SEARCH_DIR} \( $ALLSEARCHFILES \) $SEARCH_OPTIONS"
		$ECHO_DEBUG "debug.search.command=$SEARCH_COMMAND"
		find ${SEARCH_DIR} \( ${ALLSEARCHFILES} \) ${SEARCH_OPTIONS} >> ${CT_FILES} 2>>$UNIXCMDERR &
		spinner
	fi
}
	...
	do
		...
		if [ -f "$CMD" ]; then
			...
			if [ ! -x $CMD ] ; then
				...
				if [ ! -x $CMD ]; then
					echo_log "CT: CT-00019: ERROR:  The script, $CMD, is not executable, and the current user is unable to change permissions.\nPlease change persmisions on $CMD to be executable and re-run the script.\n" $CT_ERRORS
		...
		FLDNUM=`expr ${FLDNUM} + 1`
		$ECHO "\n$CMD ran without errors." >> $CT_ACTIONS_RESULTS_FILE
...
fileSort() {
	...
	CT_SORTED_FILES_TMP=$CT_SORTED_FILES.tmp
	NUMSORTFILE=0  
	...
	if [ $? -eq 0 ] ; then
		 grep -v -f ../resources/util/file_sort_list_unix.txt $CT_FILES > $CT_SORTED_FILES_TMP
	else
		/usr/xpg4/bin/grep -v -f ../resources/util/file_sort_list_unix.txt ../resources/util/file_sort_list_unix.txt >/dev/null 2>&1
		if [ $? -eq 0 ] ;
		then
			/usr/xpg4/bin/grep -v -f ../resources/util/file_sort_list_unix.txt $CT_FILES > $CT_SORTED_FILES_TMP
		else
			cp $CT_FILES $CT_SORTED_FILES_TMP
		fi
	fi
	
	# find unique list of files
	#sort -u $CT_SORTED_FILES_TMP > $CT_SORTED_FILES 
	cp $CT_SORTED_FILES_TMP $CT_SORTED_FILES
	NUMSORTFILE="`wc -l < $CT_SORTED_FILES`"
	rm $CT_SORTED_FILES_TMP
	
	# debug
	$ECHO_DEBUG "\ndebug.sorted.file.number=$NUMSORTFILE"
	
	$ECHO "Sorted Files=$NUMSORTFILE" >> $CT_RESULTS_FILE	
...
fileGetCTFiles() {
	...
	CT_ONLY_FILES=$CT_TMP/logs/CTonlyfiles.txt
	# debug
	$ECHO_DEBUG "\ndebug.function.fileGetCTFiles"
	NUMSORTFILE=0
	...
	do
		...
		then
			...
			if [ $? -eq 0 ] ;
			then
				echo "$FULL_PATH_FILE_NAME" >> $CT_ONLY_FILES
			fi
	...
	done < $CT_SORTED_FILES
	
	if [ "$OS_NAME" = "Linux" ] ; then
		IFS=$OLD_IFS
	fi

	cmp -s $CT_ONLY_FILES $CT_SORTED_FILES
	if [ $? -ne 0 ] ; then
		cp $CT_ONLY_FILES $CT_SORTED_FILES 2>>$UNIXCMDERR
	fi
	
	
	
	NUMSORTFILE="`wc -l < $CT_SORTED_FILES`" 2>>$UNIXCMDERR

	rm -f $CT_ONLY_FILES

	# debug
	
	$ECHO "Sorted Oracle only Files=$NUMSORTFILE" >> $CT_RESULTS_FILE	
...
fileCopy() {
	...
	$ECHO "processing.file$2.copy.data=" >> $CT_RESULTS_FILE
	$ECHO "[COPY_$TAG_BEGIN]" >> $CT_RESULTS_FILE
	
	# post command to results file
	cat $1 >> $CT_RESULTS_FILE 2>>$UNIXCMDERR	

	$ECHO "\n[COPY_$TAG_END]" >> $CT_RESULTS_FILE
...
fileTail() {
	...
	$ECHO "Processing file:$1" >> $CT_LOGS_FILE
	$ECHO "[TAIL_$TAG_BEGIN]" >> $CT_LOGS_FILE

	if [ -s $1 ]; then
	
		# post command to results file
		$TAIL $1 >> $CT_LOGS_FILE 2>>$UNIXCMDERR	
	else
		$ECHO "Log file is empty" >> $CT_LOGS_FILE 2>>$UNIXCMDERR	
	fi
...
fileTimestamp() {
	...
	$ECHO "processing.file$2.timestamp.data=" >> $CT_RESULTS_FILE
	$ECHO "[TIMESTAMP_$TAG_BEGIN]" >> $CT_RESULTS_FILE
	
	# post command to results file
	ls -la $1 >> $CT_RESULTS_FILE 2>>$UNIXCMDERR	

	$ECHO "[TIMESTAMP_$TAG_END]" >> $CT_RESULTS_FILE
}
...
fileListing() {
	...
	$ECHO "processing.file$2.listing.data=" >> $CT_RESULTS_FILE
	$ECHO "[LISTING_$TAG_BEGIN]" >> $CT_RESULTS_FILE
	
	# post command to results file
	ls -la $DIR_NAME >> $CT_RESULTS_FILE 2>>$UNIXCMDERR	
	
	$ECHO "[LISTING_$TAG_END]" >> $CT_RESULTS_FILE
}
...
bundleResults() {
	...
	chmod 700 $CT_TMP/FMW
	
	if [ -f $CT_TAR_FILE_LIST ] ; then
		pax -rw < $CT_TAR_FILE_LIST $CT_TMP/FMW/ 2>>$UNIXCMDERR
		if [ $? -ne 0 ] ; then
			while read -r file
			do 
				tmpdirname=`dirname $file`
				mkdir -p $CT_TMP/FMW/$tmpdirname
				cp $file $CT_TMP/FMW/$tmpdirname/.
			done < $CT_TAR_FILE_LIST
		fi
	fi
	
	if [ -f $CT_RMDATA_FILE_LIST ] ; then
		pax -rw < $CT_RMDATA_FILE_LIST $CT_TMP/FMW/ 2>>$UNIXCMDERR
		if [ $? -ne 0 ] ; then
			while read -r file
			do 
				tmpdirname=`dirname $file`
				mkdir -p $CT_TMP/FMW/$tmpdirname
				cp $file $CT_TMP/FMW/$tmpdirname/.
			done < $CT_RMDATA_FILE_LIST
		fi
		
		mv $CT_RMDATA_FILE_LIST $CT_RMDATA_FILE_LIST.bak
		...
		for file in `cat $CT_RMDATA_FILE_LIST.bak`
		do
			echo "$CT_TMP/FMW$file" >> $CT_TMP/logs/CTRMDATACopiesfilelist.txt
		done
...
bundleCTOutput() {
	# debug
	OUTPUT_UNIXCMDERRS=$CT_DEBUG/unixcmderrs.log
		...
		if [ "$DIRS" != "debug" ] ; then
			if [ -d $DIRS ] ; then
				...
				if [ "$EMPTYDIR" = "" ] ; then
					...
					if [ -s $CT_TAR_FILE ] ; then
						ACTION=u
					else
						# tar file does not exist so create it
						ACTION=c
					fi				
					tar ${ACTION}f $CT_TAR_FILE $DIRS 2>>$OUTPUT_UNIXCMDERRS
				fi
			fi
		else 
			if [ "$COLLECT_CT_DEBUG" = "true" ] ; then
				if [ -d $DIRS ] ; then
					cd $DIRS
					# check if compressed archive file exists, if so move older one.
					if [ -f $CT_DEBUG_TAR_FILE.bz2 ] ; then
						mv $CT_DEBUG_TAR_FILE.bz2 $CT_DEBUG_TAR_FILE.bz2.$TARTIMESTAMP
					fi
					if [ -f $CT_DEBUG_TAR_FILE.Z ] ; then
						mv $CT_DEBUG_TAR_FILE.Z $CT_DEBUG_TAR_FILE.Z.$TARTIMESTAMP
					fi
						
					tar cf $CT_DEBUG_TAR_FILE * 2>>$OUTPUT_UNIXCMDERRS
					bzip2 $CT_DEBUG_TAR_FILE 2>>$UNIXCMDERR
					if [ ${?} -ne 0 ] ; then
						compress $CT_DEBUG_TAR_FILE 2>>$UNIXCMDERR
						if [ ${?} -ne 0 ] ; then
							echo_log "CT: CT-00040: WARNING: Please note that for debugging purposes the following archive file was created in the output folder: $CT_DEBUG_TAR_FILE" $CT_WARNINGS	
						else
							rm $CT_DEBUG_TAR_FILE
							echo_log "CT: CT-00040: WARNING: Please note that for debugging purposes the following archive file was created in the output folder: $CT_DEBUG_TAR_FILE.Z" $CT_WARNINGS	
						fi
					else
						echo_log "CT: CT-00040: WARNING: Please note that for debugging purposes the following archive file was created in the output folder: $CT_DEBUG_TAR_FILE.bz2" $CT_WARNINGS	
					fi
					cd ..
				fi
			fi
...
fileAction() {
	...
	do 
		...
		$ECHO "Processing file $NUMPROCESSFILE path="$PROCESS_FILE_NAME"" >> $CT_RESULTS_FILE	
		$ECHO "Processing file $NUMPROCESSFILE name=$PRODUCT_FILE_BASENAME" >> $CT_RESULTS_FILE
		...
		do
			...
			case $PRODUCT_FILE_BASENAME in
				$BUNDLE_FILE)	$ECHO "$FILE_NAME" >> $CT_TAR_FILE_LIST 
				$ECHO "Tar up $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			...
			case $PRODUCT_FILE_BASENAME in
				$RMDATA_FILE)	$ECHO "$FILE_NAME" >> $CT_RMDATA_FILE_LIST 
				$ECHO "Tar up $FILE_NAME and removed sensitve data." >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			case $PRODUCT_FILE_BASENAME in
				...
				$ECHO "Getting Timestamp for $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			case $PRODUCT_FILE_BASENAME in
				...
				$ECHO "Tail $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			case $PRODUCT_FILE_BASENAME in
				...
				$ECHO "Listing directory $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			case $PRODUCT_FILE_BASENAME in
				...
				$ECHO "Copying file $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE
				FILE_PROCESSED=TRUE
				break
				;;
			esac
		...
		do
			case $PRODUCT_FILE_BASENAME in
				...
				if [ ${LOG_FILE_FLAG} = "true" ] ; then
					$ECHO "$FILE_NAME" >> $CT_TAR_FILE_LIST
					$ECHO "Tar up $FILE_NAME" >> $CT_ACTIONS_RESULTS_FILE	 
				else
  	...
  	read ANSWER
		...
		case "${ANSWER}" in
			Y|y)
				...
				;;
			N|n)
				echo_log "CT: CT-00019: ERROR: You must accept the license agreement to continue." $CT_ERRORS
				exit 1     # FALSE
				;;
			#
			# An invalid choice was entered, reprompt.
...
printMachineInfo() {
	...
	TEMPSCMD=`$PSCMD | grep java | grep -v grep |grep -v awk | awk '{print "Process# ", NR, " : ", $0} END {if (NR == 0) print ("java process not running")}'`
	echo "$TEMPSCMD" | sed -f "../resources/util/common/bin/rmdata.sed"

	TEMPSCMD=`$PSCMD_UCB | grep java | grep -v grep |grep -v awk | awk '{print "Process# ", NR, " : ", $0} END {if (NR == 0) print ("java process not running")}'`
...
printResults() {
	...
	if [ "$DEBUG" = "true" ]
	then
		$ECHO "processing.file.debug.data=$DEBUG_FILE"		
		$ECHO "processing.file.error.data=cat $UNIXCMDERR" 2>/dev/null		
	fi
	
	$ECHO "\nThe Oracle Collection tool has finished.\n"		
...
maskResults() {
	...
	mkdir $CT_TMP/masking 
	cp $CT_TAR_FILE $CT_TMP/masking/.
	# untar file
	# gnu tar supports the -C option so use pax for other *NIXs
	if [ "$OS_NAME" = "Linux" ] ; then
		tar xf $CT_TMP/masking/Collection-${MACHINE_NAME}${PRODUCTSRUN}.tar -C  $CT_TMP/masking/
	else
		cd $CT_TMP/masking/
		pax -r -s ',^/,$CT_TMP/masking/,' -f Collection-${MACHINE_NAME}${PRODUCTSRUN}.tar
		cd $PREV_DIR
	fi
	
	#delete file
	rm $CT_TMP/masking/Collection-${MACHINE_NAME}${PRODUCTSRUN}.tar
	#mask data in temp dir
	perl ./maskdata.pl $CT_TMP/masking/ $MASK_DATA
	if [ ${?} -ne 0 ] ; then
		echo_log "CT: CT-00021: ERROR: Perl error while masking data occured.  Please see documentation for troubleshooting instructions." $CT_ERRORS
	else
		CT_TAR_FILE=${OUTPUT_DIR}/Collection-${MACHINE_NAME}${PRODUCTSRUN}-masked.tar	 
		#tar up masked files
		tar cf $CT_TAR_FILE $CT_TMP/masking/ 2>>$UNIXCMDERR
		$ECHO "\nMasking data completed."
	fi
	#rm tempdir
	rm -rf $CT_TMP/masking/
...
# Por un tema de SOX    se reemplaza el /tmp por /Auditoria
#TMPDIR=/tmp
# Creamos directorio, damos permisos y seteamos variable de ambiente - by Checha
mkdir /Auditoria
chmod 777 /Auditoria
TMPDIR=/Auditoria
CT_TMP=${TMPDIR}/ct_tmp_${MACHINE_NAME}_${CT_PID}
...
if [ ! -z "$REMOTEINFO" ] ; then
	if [ ! -x ../resources/util/common/bin/recog.sh ] ; then
		...
		if [ ! -x ../resources/util/common/bin/recog.sh ]; then
			echo_log "CT: CT-00020: ERROR:  The script, ../resources/util/common/bin/recog.sh, is not executable, and the current user is unable to change permissions.\nPlease change persmisions on ../resources/util/common/bin/recog.sh to be executable and re-run the script.\n" $CT_ERRORS
			FLDNUM=`expr ${FLDNUM} + 1`
			continue
		fi
		export REMOTEINFO
	...
	$ECHO "\nSetting up lists for Collection Tool...."
	parseProductFiles
	...
	printMachineInfo > $CT_MACHINFO_FILE 2>>$UNIXCMDERR
	...
	SEARCH_FINISH=$NOW
	if [ "${ALLSEARCHFILES}" != "" ] ; then
		echo_log "\nCT file search started at $SEARCH_START and finished at $SEARCH_FINISH." $CT_ACTIONS_RESULTS_FILE		
	fi
	...
	$ECHO "[BEGIN SEARCH INFO]" >> $CT_MACHINFO_FILE
	$ECHO "Search start=$SEARCH_START" >> $CT_MACHINFO_FILE
	$ECHO "Search command=$SEARCH_COMMAND" >> $CT_MACHINFO_FILE
	$ECHO "Search finish=$SEARCH_FINISH" >> $CT_MACHINFO_FILE
	$ECHO "Search nfs=$SEARCH_NFS" >> $CT_MACHINFO_FILE
	$ECHO "[END SEARCH INFO]" >> $CT_MACHINFO_FILE
	...
	printResults  >> $CT_RESULTS_FILE 2>> $UNIXCMDERR

	# Add the compare_result info file to the tar file
	if [ -f compare_result.txt ] ; then
		cp compare_result.txt $CT_TMP/logs/.
	fi

	TARTIMESTAMP=`date '+%Y%m%d_%H%M%S'`
	# check if file exists, if so move older one.
	if [ -f $CT_TAR_FILE ] ; then
		mv $CT_TAR_FILE $CT_TAR_FILE.$TARTIMESTAMP
	fi	

	# bundle the files
	if [ -f $CT_TAR_FILE_LIST -o -f $CT_RMDATA_FILE_LIST ] ; then
		bundleResults
	fi
	
		# delete the tmp files
	if [ "$DEBUG" != "true" ] ; then
		rm -rf $CT_FILES $CT_SORTED_FILES $DEBUG_FILE  $CT_TAR_FILE_LIST 1> /dev/null 2>&1
	fi
	...
	case $ALLPRODLIST in
		*WLS* )
			for FMW_DOMAINS in `grep "Tar up .*domains.*config.xml.*" $CT_ACTIONS_RESULTS_FILE | awk '{print $3}' | sed 's/config.*//'`
			do
				echo_log "CT: CT-00200: COLLECTED: FMW configuration files for $FMW_DOMAINS" $CT_COLLECTED
			done
			;;
	esac
	
	
	for logFiles in $CT_TMP/logs/*_collected.log
	do
		echo "## CT ## Collected" > $CT_TMP/logs/results_summary.log
		if [ -s $logFiles ] ; then
			cat $logFiles >> $CT_TMP/logs/results_summary.log 2>>$UNIXCMDERR
			echo >> $CT_TMP/logs/results_summary.log
		fi
	done

	for logFiles in $CT_TMP/logs/*_warnings.log
	do
		echo "## CT ## Warnings" >> $CT_TMP/logs/results_summary.log
		if [ -s $logFiles ] ; then
			cat $logFiles >> $CT_TMP/logs/results_summary.log 2>>$UNIXCMDERR
			echo >> $CT_TMP/logs/results_summary.log
		fi
	...
	for logFiles in $CT_TMP/logs/*_errors.log
	do
		echo "## CT ## Errors" >> $CT_TMP/logs/results_summary.log
		if [ -s $logFiles ] ; then
			cat $logFiles >> $CT_TMP/logs/results_summary.log 2>>$UNIXCMDERR
			echo >> $CT_TMP/logs/results_summary.log
		fi
	...
	#move debug/non neccessary log files to CT_DEBUG
	CT_ACTIONS_RESULTS_FILE=$CT_DEBUG/Collection-Actions-${MACHINE_NAME}.txt	
	UNIXCMDERR=$CT_DEBUG/unixcmderrs.log
	...
	for DEBUG_FILENAMES in $CT_TMP/logs/*.*
	do
	...
	CT_COLLECTED=$CT_DEBUG/CT_collected.log
	CT_WARNINGS=$CT_DEBUG/CT_warnings.log
	CT_ERRORS=$CT_DEBUG/CT_errors.log
	UNIXCMDERR=$CT_DEBUG/unixcmderrs.log
	
	if [ "$PRODLIST" = " CPUQ" ] ; then
		rm -rf $CT_TMP/logs
	fi
	
	TARTIMESTAMP=`date '+%Y%m%d_%H%M%S'`

	CT_DEBUG_TAR_FILE="${OUTPUT_DIR}/debug_Collection-${MACHINE_NAME}${PRODUCTSRUN}.tar"
	...
	# check if compressed archive file exists, if so move older one.
	if [ -f $CT_TAR_FILE.bz2 ] ; then
		mv $CT_TAR_FILE.bz2 $CT_TAR_FILE.bz2.$TARTIMESTAMP
	fi
	if [ -f $CT_TAR_FILE.Z ] ; then
		mv $CT_TAR_FILE.Z $CT_TAR_FILE.Z.$TARTIMESTAMP
	fi
	...
	bzip2 $CT_TAR_FILE 2>>$UNIXCMDERR
	if [ ${?} -ne 0 ] ; then
		compress $CT_TAR_FILE 2>>$UNIXCMDERR
		if [ ${?} -ne 0 ] ; then
			echo_log "\nCould not compress Results File ${CT_TAR_FILE}." $CT_ACTIONS_RESULTS_FILE
			COMPRESSED_NAME=${CT_TAR_FILE}		
		else
			rm $CT_TAR_FILE
			echo_log "\nSuccessfully compressed the results file ${CT_TAR_FILE}." $CT_ACTIONS_RESULTS_FILE
			COMPRESSED_NAME=${CT_TAR_FILE}.Z	
		fi
	else
		echo_log "\nSuccessfully compressed the results file ${CT_TAR_FILE}." $CT_ACTIONS_RESULTS_FILE
		COMPRESSED_NAME=${CT_TAR_FILE}.bz2
	fi
	
	clear 2>>$UNIXCMDERR
	
	if [ "$CTRECOG" = "false" ] ; then
		echo_log "\nCollection process is completed, please forward ${COMPRESSED_NAME} to your assigned consultant." $CT_ACTIONS_RESULTS_FILE
	fi
	...
	ECHO_HEADER="true"
	echo
	for logFiles in $CT_DEBUG/*_collected.log
	do
		if [ -s $logFiles ] ; then
	...
	ECHO_HEADER="true"
	echo
	for logFiles in $CT_DEBUG/*_warnings.log
	do
		if [ -s $logFiles ] ; then
			if [ "$ECHO_HEADER" = "true" ] ; then
				echo "## CT ## Warnings"
	...
	ECHO_HEADER="true"
	echo
	for logFiles in $CT_DEBUG/*_errors.log
	do
		if [ -s $logFiles ] ; then
			if [ "$ECHO_HEADER" = "true" ] ; then
				echo "## CT ## Errors"
