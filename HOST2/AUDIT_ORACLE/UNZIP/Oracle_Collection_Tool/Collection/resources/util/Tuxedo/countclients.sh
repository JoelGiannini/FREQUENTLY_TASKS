...
tuxcredentialValidation () {
	...
	if [ "${SCRIPT_USER}" != "root" -o "${SCRIPT_USER}" != "${TUXDIRUSER}" ] ; then
		...
		$ECHO_COUNTCLIENT "Running the Collection Script with insufficient privileges may have a significant impact on the quality of the data and information collected from this environment. Due to this, Oracle may have to get back to you and ask for additional items, or to execute again."
		...
		if [ "${ANSWER}" = "n" ] ; then
			$ECHO_COUNTCLIENT "\nUser stopped the the Collection Script." >&2
			exit 0
		fi
	fi
...
if [ ${RETVAL} -ne 0 ]
then
	$ECHO_COUNTCLIENT "Highwater mark couldn't be located, please provide to Oracle the latest ULOG file of the Tuxedo domain\n"
fi
}
...
checkForTux()
{
	...
	then 
		...
		do
			...
			$ECHO_COUNTCLIENT "Collection Tool, enter [quit or q]:" >&2
			read TUXDIR
		...
		if [ "${QUIT_COUNTCLIENTS}" = "yes" ]; then
			$ECHO_COUNTCLIENT "User chose to quit countclients script." >> $CT_TMP\${MACHINE_NAME}-countclients.txt
		else	
			tuxcredentialValidation
		
			$ECHO_COUNTCLIENT "TUXDIR=${TUXDIR}" >> $CT_TMP\${MACHINE_NAME}-countclients.txt
			. ${TUXDIR}/tux.env
			...
			if [ "${QUIT_COUNTCLIENTS}" = "yes" ]; then
				...
				$ECHO_COUNTCLIENT "TUXCONFIG=${TUXCONFIG}" >> $CT_TMP\${MACHINE_NAME}-countclients.txt

				getconfigmodel >> $CT_TMP\${MACHINE_NAME}-countclients.txt
				if [ "${tux_model}" = "SHM" ]
				then
					getSHMboottime >> $CT_TMP\${MACHINE_NAME}-countclients.txt
				else
					getMPboottime >> $CT_TMP\${MACHINE_NAME}-countclients.txt
				fi
				gethighwatercnt >> $CT_TMP\${MACHINE_NAME}-countclients.txt
				RETVAL=${?}
				if [ ${RETVAL} -ne 0 ]
				then
					$ECHO_COUNTCLIENT "Could not connect to the running application to get the Tuxedo client count."  >> $CT_TMP\${MACHINE_NAME}-countclients.txt			
				fi
				$TUXDIR/bin/tmunloadcf >> $CT_TMP\${MACHINE_NAME}-countclients.txt
				RETVAL=${?}
				if [ ${RETVAL} -ne 0 ]
				then
					$ECHO_COUNTCLIENT "Could not get application configuration, please provide to Oracle the latest ULOG file of the Tuxedo domain" >> $CT_TMP\${MACHINE_NAME}-countclients.txt
					$ECHO_COUNTCLIENT "Could not get application configuration, please provide to Oracle the latest ULOG file of the Tuxedo domain"							
				fi
				exit 0
			fi
		fi
	else
		$ECHO_COUNTCLIENT "No Tuxedo BBL processes found on this machine."  >> $CT_TMP\${MACHINE_NAME}-countclients.txt
		$ECHO_COUNTCLIENT "Countclients configuration gathering script not run."  >> $CT_TMP\${MACHINE_NAME}-countclients.txt
...
STANDALONE=
# check to see if Collection is running, if not then print license.
