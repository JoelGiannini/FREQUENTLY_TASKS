...
:: check to see if Collection is running, if it is, don't print the license prompt again
tasklist /v | findstr /C:"Collection Tool"
echo er %errorlevel%
if errorlevel 1 (
  goto printLicense
) else (
...
if "%TUXDIR%" == "q" (
	echo User chose to quit countclients.>> %CT_TMP%\%COMPUTERNAME%-countclients.txt
	goto endcountclients
) else if "%TUXDIR%" == "quit" (
	echo User chose to quit countclients.>> %CT_TMP%\%COMPUTERNAME%-countclients.txt
	goto endcountclients
...
:: init variables
set SCRIPT_VERSION=19.1
set PATH=%PATH%;%TUXDIR%\bin


tasklist /v | findstr /C:"BBL" >> %CT_TMP%\%COMPUTERNAME%-countclients.txt
echo er %errorlevel%
if errorlevel 1 (
  goto BBLnotFound
) else (
...
REM
type temp.txt|ud32 -C tpsysadm >>%CT_TMP%\%COMPUTERNAME%-countclients.txt

del temp.txt

tmunloadcf >> %CT_TMP%\%COMPUTERNAME%-countclients.txt

goto endcountclients

:BBLnotFound
echo "No Tuxedo BBL processes found running on this machine.  Countclients will not be run."
echo "No Tuxedo BBL processes found running on this machine.  Countclients will not be run." >> %CT_TMP%\%COMPUTERNAME%-countclients.txt
