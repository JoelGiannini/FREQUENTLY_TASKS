...
if [ "${QUIT_WLST}" = "yes" ]; then
	...
	echo_nup_log "CT: WLS-03001: WARNING: User chose to quit WLS NUP measurement script." $WLS_WARNINGS
	
	rmdir $CT_TMP/WLSNUP 2>/dev/null
else
	...
	if [ ! -d $CT_TMP/WLSNUP ] ; then
		mkdir -p $CT_TMP/WLSNUP 
	fi	
