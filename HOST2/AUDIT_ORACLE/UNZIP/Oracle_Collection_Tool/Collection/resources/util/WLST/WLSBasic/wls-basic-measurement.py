...
def getWLBasic_Info():
	...
	WLSBasic_ERRORS = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_errors.log')
	fosWLSBasicerr = FileOutputStream(WLSBasic_ERRORS,true)

	WLSBasic_WARNINGS = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_warnings.log')
	fosWLSBasicwarn = FileOutputStream(WLSBasic_WARNINGS,true)

	WLSBasic_COLLECTED = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_collected.log')
	...
	while 1:
		...
		try: 
			...
			if WLST_return.find('authenticated') >= 0:
				print '\n\nWLS: CT-03003: ERROR: WLBasic script failed to authenticate user.  Invalid userID or password.'
				quitWLST = raw_input('Please press enter to try to connect again with new credentials or q to quit:')
					
				if quitWLST.strip() == 'q':
					Failed_output = File(os.environ["CT_TMP"] + os.sep + 'WLSBasic' + os.sep + targetMachine + '_OracleWLS_WL_Basic.txt')
					fosFailedOut = FileOutputStream(Failed_output)	
					
					theInterpreter.setOut(fosWLSBasicerr)
					print 'WLS: CT-03003: ERROR: WLBasic script failed to authenticate user.  Invalid userID or password.'
				...
				print 'WLS: CT-03004: ERROR: WLBasic script failed to connect to URL.  Please check the URL and port and try again.'
				quitWLST = raw_input('Please press enter to try to connect again with new credentials or q to quit:')
	...
	if eval(verElements[0]) < 10 :
		...
		theInterpreter.setOut(fosWLSBasicerr)			
		print 'WLS: CT-03005: ERROR: WLBasic script failed to connect to URL, this version of WebLogic is not supported by the WLST WLBasic script.'
	...
	WL_Basic_output = File(os.environ["CT_TMP"] + os.sep + 'WLSBasic' + os.sep + fBaseName)
	...
	print 'WLS: CT-03201: COLLECTED: WLBasic measurement running on ' + targetMachine + ' Domain ' + domainName
