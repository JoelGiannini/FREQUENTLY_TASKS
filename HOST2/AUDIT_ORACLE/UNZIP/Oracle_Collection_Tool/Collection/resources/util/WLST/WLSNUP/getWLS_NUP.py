...
NUP_script_version = "19.1"

def getNUP_Info():

	NUP_ERRORS = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_errors.log')
	fosNUPerr = FileOutputStream(NUP_ERRORS,true)

	NUP_WARNINGS = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_warnings.log')
	fosNUPwarn = FileOutputStream(NUP_WARNINGS,true)

	NUP_COLLECTED = File(os.environ["CT_TMP"] + os.sep + 'logs' + os.sep + 'WLS_collected.log')
	...
	while 1:
		...
		try: 
			...
			if WLST_return.find('authenticated') >= 0:
				print '\n\nWLS: CT-03000: ERROR: WLSNUP script failed to authenticate user.  Invalid userID or password.'
				quitWLST = raw_input('Please press enter to try to connect again with new credentials or q to quit:')
				
				if quitWLST.strip() == 'q':
					Failed_output = File(os.environ["CT_TMP"] + os.sep + 'WLSNUP' + os.sep + targetMachine + '_OracleWLS_NUP.txt')
					fosFailedOut = FileOutputStream(Failed_output)
					theInterpreter.setOut(fosNUPerr)
					print 'WLS: CT-03000: ERROR: WLSNUP script failed to authenticate user.  Invalid userID or password.'
				...
				print '\n\nWLS: CT-03001: ERROR: WLSNUP script failed to connect to URL.  Please check the URL and port and try again.'
				quitWLST = raw_input('Please press enter to try to connect again with new credentials or q to quit:')
				...
				if quitWLST.strip() == 'q':
					...
					fosFailedOut = FileOutputStream(Failed_output)				
					theInterpreter.setOut(fosNUPerr)
					print 'WLS: CT-03001: ERROR: WLSNUP script failed to connect to URL.  Please check the URL and port and try again.'
					theInterpreter.setOut(fosFailedOut)
					print 'WLS: CT-03001: ERROR: WLSNUP script failed to connect to URL.  Please check the URL and port and try again.'
					print
					print 'End of getWLS_NUP'
	...
	if eval(verElements[0]) < 9 :
		...
		theInterpreter.setOut(fosNUPerr)
		print 'WLS: CT-03002: ERROR: WLSNUP script failed to connect to URL, this version of WebLogic is not supported by the WLST NUP script.'
		theInterpreter.setOut(fosFailedOut)
		print 'WLS: CT-03002: ERROR: WLSNUP script failed to connect to URL, this version of WebLogic is not supported by the WLST NUP script.'
	...
	NUP_output = File(os.environ["CT_TMP"] + os.sep + 'WLSNUP' + os.sep + fBaseName)
	...
	print 'WLS: CT-03200: COLLECTED: NUP measurement running on ' + targetMachine + ' Domain ' + domainName
