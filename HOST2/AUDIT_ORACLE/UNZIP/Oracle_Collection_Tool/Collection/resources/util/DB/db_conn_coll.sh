...
if [ ! -s checkspace.txt ]; then
	CT_NOSPACE=1
	CT_NOSPACEFOLDER=`pwd`
	echo "DB: CT-02019: ERROR: No disk space available on ${CT_NOSPACEFOLDER} folder"
...
if [ $chkoh -eq 1 ]; then
 ...
 if [ "$yn" = "n" ]; then
	...
	echo  "CT-02010 : Access error on ORACLE_HOME: ${V_ORACLE_HOME}" >>$log_err
	continue
 else
	V_ORACLE_HOME=""
	check_oracle_home
...
check_space
if [ $CT_NOSPACE -eq 1 ];then
    logwrite  "DB: CT-02019: ERROR: No disk space available on ${CT_NOSPACEFOLDER} folder"
	grep 'CT-02019' $log_err >/dev/null 2>&1
    if [ $? -eq 1 ] ; then
		echo "DB: CT-02019: ERROR: No disk space available on ${CT_NOSPACEFOLDER} folder" >>$log_err
	fi
	exit 
fi
	...
	if [ $? -eq 0 ]; then
		...
		if [ "$yn" = "n" ]; then
			...
			if [ !  "$REMOTE_DB" = "YES" ];then
				echo "DB: CT-02011: Failed to connect to instance ${V_ORACLE_SID}: `egrep 'ORA-[0-9]*:' checkconn.log`"  >>$log_err
			fi
			rm -f checkconn.log
			continue
		else
...
SCRIPT_VERSION=$CT_BUILD_VERSION
 ...
 if [ "$V_CONNECTION_METHOD" = "LOCAL" ]; then
	...
	log_write "Start colecting data for local instance $V_ORACLE_SID"
	 ...
	 if [ $status -eq 18 ]; then 
	   grep 'CT-02019' $log_err >/dev/null 2>&1
       if [ $? -eq 1 ] ; then
			echo "DB: CT-02019: ERROR: No disk space available" >>$log_err
	   fi
	   exit
	 fi	
