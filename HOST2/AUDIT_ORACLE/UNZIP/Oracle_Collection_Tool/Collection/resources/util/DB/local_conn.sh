...
if [ ! -s checkspace.txt ]; then
	CT_NOSPACE=1
	echo "DB: CT-02019: ERROR: No disk space available on ${CT_TMP} folder"
...
if [ $chksqlp -eq 1 ]; then
 if [ "$LICAGREE" = "YES" ]; then
	...
	echo  "DB: CT-02010: Access error on ORACLE_HOME: ${V_ORACLE_HOME}" >>$log_err
	exit
 else
...
if [ "$V_OS_USER" != "$USR_ID" ] ; then
 if [ $USR_ID != "root" ] && [ "$LICAGREE" = "YES" ]; then
	...
	if [ ! -w $V_TMP ]; then
		...
		grep 'CT-02013' "$log_err" >/dev/null 2>&1
        if [ $? -eq 1 ] ; then
			echo "DB: CT-02013: No writing permission on $V_TMP folder"  	>>"$log_err"
		fi	
		exit 201
	fi	
	...
	if [ ! -x $V_TMP ]; then
		...
		echo "Failed to connect to local instance ${V_ORACLE_SID}, ORACLE_HOME=${V_ORACLE_HOME}, SQL=${SQL_SCRIPT}" >>"$logfile"
		grep 'CT-02012' "$log_err" >/dev/null 2>&1
        if [ $? -eq 1 ] ; then
			echo  "DB: CT-02012: No execute permission on $V_TMP folder" 	>>"$log_err"
		fi
		exit 201
	fi		
 ...
 if [ $CT_NOSPACE -eq 1 ];then
    ...
    if [ $? -eq 1 ] ; then
		echo "DB: CT-02019: ERROR: No disk space available on ${CT_TMP} folder" >>"$log_err"
	fi
	exit 18
 fi	
 ...
 if [ ! -f checkconn.txt ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    rm -f checkconn.txt
	 ...
	 echo "DB: CT-02011: Failed to connect to instance ${V_ORACLE_SID}: `egrep 'ORA-|SP2-' checkconn.txt`"  >>${log_err}
