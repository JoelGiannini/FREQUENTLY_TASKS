...
SET PAUSE OFF
SET TERMOUT OFF
REM     ORACLE - Global Licensing and Advisory Services - EBS Collection Script
REM
REM     Change History
REM     ---------------------------------
...
--#<CT>#define SCRIPT_LA=''                   -- script prompts for license agreement [default behavior]
-- Set SCRIPT_SI to run in interactive or in silent mode
--#<CT>#define SCRIPT_SI=_SI_IGNORE_THIS_ERR  -- script does not prompt for privilege check confirmation
--#<CT>#define SCRIPT_SI=''                   -- script prompts for privilege check confirmation [default behavior]
-- Set SCRIPT_SD to create output subdirectory
define SCRIPT_SD=EBS                  -- create output subdirectory
--#<CT>#define SCRIPT_SD=''                   -- no output subdirectory



-- PREPARE AND DISPLAY LICENSE AGREEMENT

SPOOL&SCRIPT_LA license_agreement.txt
PROMPT ===============================================================
...
PROMPT ===============================================================
PROMPT
PROMPT Terms for Oracle Software Collection Tool
PROMPT
PROMPT
...
PROMPT
PROMPT PLEASE SCROLL DOWN AND READ ALL OF THE FOLLOWING TERMS AND CONDITIONS OF THIS LICENSE AGREEMENT (this "Agreement") CAREFULLY.  THIS AGREEMENT IS A LEGALLY BINDING CONTRACT BETWEEN YOU AND ORACLE AMERICA, INC. THAT SETS FORTH THE TERMS THAT GOVERN YOUR USE OF THE SOFTWARE.
PROMPT
PROMPT YOU MUST ACCEPT AND ABIDE BY THESE TERMS AS PRESENTED TO YOU - ANY CHANGES, ADDITIONS OR DELETIONS BY YOU TO THESE TERMS ARE NOT ACCEPTED  AND WILL NOT BE PART OF THIS AGREEMENT.
PROMPT
PROMPT Definitions
PROMPT "Oracle" refers to Oracle America, Inc.
PROMPT
PROMPT "You" and "Your" refers to the individual or entity that wishes to use the Software.
PROMPT
PROMPT "Software" refers to the tool(s), script(s) and/or software product(s) (and any applicable documentation) provided with these terms to You by Oracle and which You wish to access and use to measure, monitor and/or manage Your usage of separately-licensed Oracle software (the "Programs") that has been licensed under a separate agreement between Oracle and You, such as an Oracle Master Agreement, an Oracle Software License and Services Agreement, an Oracle PartnerNetwork Agreement or an Oracle distribution agreement (each, an "Oracle License Agreement").
PROMPT
...
PROMPT -	make the Software available in any manner to any third party for use in the third party's business operations, without Oracle's prior written consent;
...
PROMPT -	cause or permit reverse engineering (unless required by law for interoperability), disassembly or decompilation of the Software (the foregoing prohibition includes but is not limited to review of data structures or similar materials produced by the Software);
PROMPT -	disclose results of any Software benchmark tests without Oracle's prior written consent; 
PROMPT -	use any Oracle name, trademark or logo without Oracle's prior written consent.  
PROMPT
...
PROMPT You may terminate this Agreement by destroying all copies of the Software.  Oracle has the right to terminate Your right to use the Software at any time upon notice to You, in which case You shall destroy all copies of the Software.
PROMPT
...
PROMPT 
PROMPT 5.	This Agreement may not be modified and the rights and restrictions may not be altered or waived except in a writing signed by authorized representatives of You and Oracle.  
PROMPT 
PROMPT 6.	Any notice required under this Agreement shall be provided to the other party in writing.  
PROMPT 
...
PROMPT Should You have any questions concerning Your use of the Software or this Agreement, please contact Oracle at: http://www.oracle.com/corporate/contact/
PROMPT
PROMPT Oracle America, Inc.
PROMPT 500 Oracle Parkway,
PROMPT Redwood City, CA 94065
PROMPT
PROMPT Last updated 1 November 2018
PROMPT
PROMPT ===============================================================

SPOOL OFF

--#<CT>#HOST&SCRIPT_LA more license_agreement.txt   2> fii_err.txt
...
--#<CT>#HOST&SCRIPT_LA rm   license_agreement.txt   2> fii_err.txt
--#<CT>#HOST&SCRIPT_LA del  license_agreement.txt   2> fii_err.txt
...
--#<CT>#--{
--#<CT>#--Detect SQL*Plus client path separator
--#<CT>#--Using some Unix/Linux specific syntax
--#<CT>#host echo select \'$PWD\' as PWD_, \'rm\' as RMDEL_, \'/\' as PSEP_ from dual where \'$PWD\' like \'%/%\'\; > psep.sql 2> fii_err.txt
--#<CT>#
--#<CT>#define PWD=*
--#<CT>#define RMDEL=del
--#<CT>#define PSEP=\
--#<CT>#col PWD_   new_val PWD   noprint
--#<CT>#col RMDEL_ new_val RMDEL noprint
--#<CT>#col PSEP_  new_val PSEP  noprint
--#<CT>#-- The query syntax is correct only on Unix/Linux
--#<CT>#SET TERMOUT OFF
--#<CT>#@psep.sql
--#<CT>#SET TERMOUT ON
--#<CT>#-- Cleanup
--#<CT>#host &RMDEL psep.sql   2> fii_err.txt
--#<CT>#--}
...
PROMPT Collection Tool version &&CT_V
PROMPT
...
prompt
prompt ==============
prompt ***Collection Tool version

define USAGE_QUERY=CT_V
...
BEGIN
  ...
  dbms_output.put_line('EBS: CT-06000: COLLECTED: EBS Repository: ' || INSTANCE_N || PDB_N || ' running on Host: ' || HOST_N);
END;
/
SPOOL OFF
--#<CT># }
