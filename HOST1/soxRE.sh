...
then
	if [[ `${SSH} ibmunx@$i "sudo cat /etc/redhat-release | grep 8.*"`  == "" ]]; then
			...
			$SCP /home/ibmunx/JOEL/HOST1/sox_tasa_linux_v2.5.sh ibmunx@$i:/SOX
			...
			${SSH} ibmunx@$i "sudo mkdir /SOX 2>/dev/null && sudo chmod 7777 /SOX"
 			$SCP /home/ibmunx/JOEL/HOST1/sox_tasa_linux_rhel8_v3.1.sh ibmunx@$i:/SOX
	...
	echo "Es ubuntu"
		...
		${SSH} ibmunx@$i "sudo mkdir /SOX 2>/dev/null && sudo chmod 7777 /SOX"
		$SCP /home/ibmunx/JOEL/HOST1/sox_remediacion_AIX_v2.1.sh ibmunx@$i:/SOX
